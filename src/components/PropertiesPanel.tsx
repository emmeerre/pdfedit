    ...
    <div className="w-72 bg-white border-l border-slate-200 flex flex-col h-full overflow-y-auto">
      ...
      <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/70">
        <div className="flex items-center gap-2">
          ...
          {el.type === 'dropdown' && <ListFilter className="w-4 h-4 text-indigo-600" />}
          ...
          <span className="font-semibold text-xs uppercase tracking-wider text-slate-700">
            ...
            {el.type === 'dropdown' && 'Menu a Tendina'}
      ...
      <div className="p-4 space-y-4 flex-1">
        ...
        {el.type === 'dropdown' && (
          <>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Opzioni Selezionabili (una per riga)
              </label>
              <textarea
                rows={4}
                ...
                onChange={(e) => {
                  ...
                  onUpdateElement({
                    ...el,
                    options: opts,
                    defaultValue: opts.includes(el.defaultValue) ? el.defaultValue : opts[0] || '',
                  });
                }}
                placeholder="Opzione 1&#10;Opzione 2&#10;Opzione 3"
                className="w-full px-2.5 py-1.5 text-xs border border-slate-300 rounded-md font-sans"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Valore Selezionato di Default
              </label>
              <select
                value={el.defaultValue}
                onChange={(e) => onUpdateElement({ ...el, defaultValue: e.target.value })}
                className="w-full px-2.5 py-1.5 text-xs border border-slate-300 rounded-md bg-white font-medium cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    onUpdateElement({ ...el, fontSize: parseFloat(e.target.value) || 10 })
                  }
                  className="w-full px-2.5 py-1.5 text-xs border border-slate-300 rounded-md"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="color"
                  value={el.fontColor || '#0f172a'}
                  onChange={(e) => onUpdateElement({ ...el, fontColor: e.target.value })}
            ...
            <div className="pt-2 border-t border-slate-100">
              <label className="flex items-center gap-2 cursor-pointer p-2 bg-slate-50 rounded-lg border border-slate-200">
                <input
                  type="checkbox"
                  checked={Boolean(el.isRequired)}
                  onChange={(e) => onUpdateElement({ ...el, isRequired: e.target.checked })}
                  className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 w-4 h-4"
                />
                <span className="text-xs font-medium text-slate-800">Campo Obbligatorio</span>
              </label>
            </div>
          </>
        )}
        ...
        {/* Shape / Line / Rectangle / Original Decoration Controls */}
        {el.type === 'shape' && (
          <div className="space-y-3">
            {el.isOriginalPdfDecoration && (
              <div className="p-2.5 bg-purple-50 border border-purple-200 rounded-md">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-purple-800 mb-1">
                  ...
                  <span>Grafica Originale PDF</span>
                </div>
                <p className="text-[11px] text-purple-700 leading-snug">
                  Stai modificando una forma o linea estratta direttamente dal PDF originale. Puoi cambiarne colore, spessore, dimensioni o spostarla.
                </p>
                {el.originalX !== undefined && (
                  <button
                    type="button"
                    onClick={() =>
                      onUpdateElement({
                        ...
                        rotation: el.originalRotation || 0,
                      })
                    }
                    className="mt-2 text-[11px] text-purple-700 hover:text-purple-900 bg-white border border-purple-200 px-2 py-1 rounded w-full flex items-center justify-center gap-1 font-medium transition-colors"
                  >
                    <RefreshCw className="w-3 h-3" />
                    <span>Ripristina coordinate originali</span>
                  </button>
                )}
              </div>
            )}
            ...
            {/* Shape Type Selector */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Tipo di Forma</label>
              <div className="grid grid-cols-3 gap-1">
                {(
                  [
                    { id: 'rectangle', label: 'Riquadro' },
                    { id: 'line', label: 'Linea' },
                  ...
                  <button
                    key={st.id}
                    type="button"
                    onClick={() => onUpdateElement({ ...el, shapeType: st.id })}
                    className={`py-1 text-xs rounded border font-medium transition-all ${
                      el.shapeType === st.id
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-semibold text-slate-700">
                  {el.shapeType === 'line' ? 'Colore Linea' : 'Colore Bordo'}
                </label>
                <div className="flex items-center gap-1">
                  <div
                    className="w-4 h-4 rounded border border-slate-300"
                    style={{ backgroundColor: el.strokeColor || '#334155' }}
                  />
                  <input
                    type="color"
                    value={el.strokeColor || '#334155'}
                    onChange={(e) => onUpdateElement({ ...el, strokeColor: e.target.value })}
              ...
              <div className="flex gap-1.5 mt-1">
                {['#0f172a', '#334155', '#2563eb', '#059669', '#dc2626', '#d97706', '#9333ea'].map((c) => (
                  <button
                    key={c}
                    type="button"
                    onClick={() => onUpdateElement({ ...el, strokeColor: c })}
                    style={{ backgroundColor: c }}
                    className={`w-5 h-5 rounded-full border border-slate-200 transition-transform ${
                      el.strokeColor === c ? 'scale-115 ring-2 ring-blue-500' : 'hover:scale-110'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Spessore ({el.strokeWidth || 1}pt)</label>
                <input
                  type="number"
                  min="0"
                  max="30"
                  value={el.strokeWidth ?? 1}
                  onChange={(e) => onUpdateElement({ ...el, strokeWidth: Math.max(0, parseInt(e.target.value) || 0) })}
                  className="w-full px-2 py-1 text-xs border border-slate-300 rounded-md"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Stile Tratto</label>
                <select
                  value={el.strokeDash || 'solid'}
                  onChange={(e) => onUpdateElement({ ...el, strokeDash: e.target.value as any })}
            ...
            {el.shapeType !== 'line' && (
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-semibold text-slate-700">Colore Riempimento</label>
                  <div className="flex items-center gap-1.5">
                    <button
                      type="button"
                      onClick={() => onUpdateElement({ ...el, fillColor: 'transparent' })}
                    ...
                    <input
                      ...
                      onChange={(e) => onUpdateElement({ ...el, fillColor: e.target.value })}
                      className="w-5 h-5 p-0 border-0 rounded cursor-pointer"
                    />
                  </div>
                </div>
                {/* Presets */}
                <div className="flex gap-1.5 mt-1">
                  {['#ffffff', '#f8fafc', '#f1f5f9', '#eff6ff', '#ecfdf5', '#fef2f2', '#fffbeb'].map((c) => (
                    <button
                      key={c}
                      type="button"
                      onClick={() => onUpdateElement({ ...el, fillColor: c })}
                      style={{ backgroundColor: c }}
                      className={`w-5 h-5 rounded-full border border-slate-300 transition-transform ${
                        el.fillColor === c ? 'scale-115 ring-2 ring-blue-500' : 'hover:scale-110'
                      }`}
                      title={c}
                    />
                  ))}
            ...
            {el.shapeType === 'rectangle' && (
              <div>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="font-semibold text-slate-700">Angoli Arrotondati</span>
                  <span className="font-mono text-slate-500 text-[11px]">{el.borderRadius || 0}px</span>
                </div>
                <input
                  type="range"
                  min="0"
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
              <div>
                <span className="text-[10px] text-slate-400 block mb-0.5">Larghezza (pt)</span>
                <input
                  type="number"
                  min="2"
                  max="1200"
                  value={Math.round(el.width)}
                  onChange={(e) => onUpdateElement({ ...el, width: Math.max(2, parseInt(e.target.value) || 2) })}
                  className="w-full px-2 py-1 text-xs border border-slate-300 rounded-md font-mono"
                />
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block mb-0.5">Altezza (pt)</span>
                <input
                  type="number"
                  min="1"
                  max="1200"
                  value={Math.round(el.height)}
                  onChange={(e) => onUpdateElement({ ...el, height: Math.max(1, parseInt(e.target.value) || 1) })}
                  className="w-full px-2 py-1 text-xs border border-slate-300 rounded-md font-mono"
                />
              </div>
            </div>
